...
export default async function decorate(block) {
  ...
  const navbuttonsRows = navbuttonsBlock
    ? [...navbuttonsBlock.querySelectorAll(':scope > div')]
    : [];
  ...
  document.addEventListener('click', (e) => {
    if (!block.contains(e.target)) {
      const anyMegaOpen = desktopNav
        && desktopNav.querySelector('.siteheader-navlink[aria-expanded="true"]');
      if (anyMegaOpen) {
        desktopNav.querySelectorAll('.siteheader-navlink[aria-expanded="true"]').forEach((link) => {
          link.setAttribute('aria-expanded', 'false');
        });
